...
abstract class BaseMessageOptions {
    ...
    public static ActionArgs toArgs(Context context2) {
        return new ActionArgs().with("Title.Text", MessageTemplates.getApplicationName(context2)).withColor("Title.Color", ViewCompat.MEASURED_STATE_MASK).with("Message.Text", "Popup message goes here.").withColor("Message.Color", ViewCompat.MEASURED_STATE_MASK).withFile("Background image", null).withColor("Background color", -1).with("Accept button.Text", "OK").withColor("Accept button.Background color", -1).withColor("Accept button.Text color", Color.argb(255, 0, 122, 255)).withAction("Accept action", null);
    }
}
