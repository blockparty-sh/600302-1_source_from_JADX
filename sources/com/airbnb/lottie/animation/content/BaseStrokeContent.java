...
public abstract class BaseStrokeContent implements AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f93pm = new PathMeasure();
    ...
    public void setContents(List<Content> list, List<Content> list2) {
        ...
        for (int size = list.size() - 1; size >= 0; size--) {
            ...
            if (content instanceof TrimPathContent) {
                ...
                if (trimPathContent2.getType() == C0870Type.INDIVIDUALLY) {
        ...
        for (int size2 = list2.size() - 1; size2 >= 0; size2--) {
            ...
            if (content2 instanceof TrimPathContent) {
                ...
                if (trimPathContent3.getType() == C0870Type.INDIVIDUALLY) {
                    if (pathGroup != null) {
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        ...
        if (C0878Utils.hasZeroScaleAxis(matrix)) {
        ...
        this.paint.setStrokeWidth(((FloatKeyframeAnimation) this.widthAnimation).getFloatValue() * C0878Utils.getScale(matrix));
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        ...
        this.f93pm.setPath(this.path, false);
        float length = this.f93pm.getLength();
        while (this.f93pm.nextContour()) {
            length += this.f93pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f93pm.setPath(this.trimPathPath, false);
            float length2 = this.f93pm.getLength();
            float f2 = 1.0f;
            if (floatValue3 > length) {
                float f3 = floatValue3 - length;
                if (f3 < f + length2 && f < f3) {
                    C0878Utils.applyTrimPathIfNeeded(this.trimPathPath, floatValue2 > length ? (floatValue2 - length) / length2 : 0.0f, Math.min(f3 / length2, 1.0f), 0.0f);
            ...
            if (f4 >= floatValue2 && f <= floatValue3) {
                if (f4 > floatValue3 || floatValue2 >= f) {
                    ...
                    C0878Utils.applyTrimPathIfNeeded(this.trimPathPath, f5, f2, 0.0f);
                    canvas.drawPath(this.trimPathPath, this.paint);
                } else {
                    canvas.drawPath(this.trimPathPath, this.paint);
                }
    ...
    private void applyDashPatternIfNeeded(Matrix matrix) {
        ...
        float scale = C0878Utils.getScale(matrix);
