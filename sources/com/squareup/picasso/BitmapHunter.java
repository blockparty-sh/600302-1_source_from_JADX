...
class BitmapHunter implements Runnable {
    ...
    static Bitmap decodeStream(Source source, Request request) throws IOException {
        ...
        boolean isWebPFile = C2643Utils.isWebPFile(buffer);
    ...
    public void run() {
        ...
        try {
            ...
            if (this.picasso.loggingEnabled) {
                C2643Utils.log("Hunter", "executing", C2643Utils.getLogIdsForHunter(this));
            }
            this.result = hunt();
            if (this.result == null) {
    ...
    public Bitmap hunt() throws IOException {
        ...
        if (MemoryPolicy.shouldReadFromMemoryCache(this.memoryPolicy)) {
            ...
            if (bitmap != null) {
                ...
                if (this.picasso.loggingEnabled) {
                    C2643Utils.log("Hunter", "decoded", this.data.logId(), "from cache");
                }
                return bitmap;
            }
        } else {
        ...
        if (bitmap != null) {
            if (this.picasso.loggingEnabled) {
                C2643Utils.log("Hunter", "decoded", this.data.logId());
            }
            ...
            if (this.data.needsTransformation() || this.exifOrientation != 0) {
                synchronized (DECODE_LOCK) {
                    if (this.data.needsMatrixTransform() || this.exifOrientation != 0) {
                        ...
                        if (this.picasso.loggingEnabled) {
                            C2643Utils.log("Hunter", "transformed", this.data.logId());
                        }
                    }
                    if (this.data.hasCustomTransformations()) {
                        bitmap = applyCustomTransformations(this.data.transformations, bitmap);
                        if (this.picasso.loggingEnabled) {
                            C2643Utils.log("Hunter", "transformed", this.data.logId(), "from custom transformations");
                        }
                    }
                }
                if (bitmap != null) {
        ...
        if (this.action == null) {
            ...
            if (z) {
                ...
                if (list == null || list.isEmpty()) {
                    C2643Utils.log(str3, str2, request.logId(), "to empty hunter");
                } else {
                    C2643Utils.log(str3, str2, request.logId(), C2643Utils.getLogIdsForHunter(this, str));
                }
            }
            return;
        }
    ...
    public void detach(Action action2) {
        ...
        if (this.picasso.loggingEnabled) {
            C2643Utils.log("Hunter", "removed", action2.request.logId(), C2643Utils.getLogIdsForHunter(this, "from "));
        }
    }
