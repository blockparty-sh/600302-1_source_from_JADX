...
public class Stetho {
    ...
    public static abstract class Initializer {
        ...
        private class RealSocketHandlerFactory implements SocketHandlerFactory {
            ...
            public SocketHandler create() {
                ...
                Iterable inspectorModules = Initializer.this.getInspectorModules();
                if (inspectorModules != null) {
                    protocolDetectingSocketHandler.addHandler(new AlwaysMatchMatcher(), new DevtoolsSocketHandler(Initializer.this.mContext, inspectorModules));
                }
        ...
        public final void start() {
            new ServerManager(new LocalSocketServer(NetworkParameters.PAYMENT_PROTOCOL_ID_MAINNET, AddressNameHelper.createCustomAddress("_devtools_remote"), new LazySocketHandler(new RealSocketHandlerFactory()))).start();
        }
    }
    ...
    public static void initialize(Initializer initializer) {
        if (!ActivityTracker.get().beginTrackingIfPossible((Application) initializer.mContext.getApplicationContext())) {
            LogUtil.m50w("Automatic activity tracking not available on this API level, caller must invoke ActivityTracker methods manually!");
        }
        initializer.start();
    }
