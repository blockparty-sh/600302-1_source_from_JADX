...
public class Analytics extends AbstractAppCenterService {
            ...
            public void run() {
                ...
                if (analyticsTransmissionTarget != null) {
                    if (analyticsTransmissionTarget.isEnabled()) {
                        ...
                        AppCenterLog.error(str, "This transmission target is disabled.");
                        return;
                    }
                } else if (!Analytics.this.mStartedFromApp) {
                    AppCenterLog.error(str, "Cannot track event using Analytics.trackEvent if not started from app, please start from the application or use Analytics.getTransmissionTarget.");
                    return;
                }
    ...
    private boolean setInstanceTransmissionInterval(int i) {
        ...
        String str = LOG_TAG;
        if (channel != null) {
            AppCenterLog.error(str, "Transmission interval should be set before the service is started.");
            return false;
        } else if (i < 3 || i > MAXIMUM_TRANSMISSION_INTERVAL_IN_SECONDS) {
            AppCenterLog.error(str, String.format(Locale.ENGLISH, "The transmission interval is invalid. The value should be between %d seconds and %d seconds (%d day).", new Object[]{Integer.valueOf(3), Integer.valueOf(MAXIMUM_TRANSMISSION_INTERVAL_IN_SECONDS), Long.valueOf(TimeUnit.SECONDS.toDays(86400))}));
            return false;
        } else {
