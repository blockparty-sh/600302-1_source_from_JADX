        ...
        if (AppCenter.getInstance().isInstanceEnabled()) {
            ...
            try {
                if (!semaphore.tryAcquire(5000, TimeUnit.MILLISECONDS)) {
                    AppCenterLog.error(str, "Timeout waiting for looper tasks to complete.");
                }
            } catch (InterruptedException e) {
                AppCenterLog.warn(str, "Interrupted while waiting looper to flush.", e);
            }
        }
        java.lang.Thread.UncaughtExceptionHandler uncaughtExceptionHandler = this.mDefaultUncaughtExceptionHandler;
