...
public final class TraceOptions {
    ...
    public static TraceOptions fromBytes(byte[] bArr) {
        C2865Utils.checkNotNull(bArr, "buffer");
        C2865Utils.checkArgument(bArr.length == 1, "Invalid size: expected %s, got %s", Integer.valueOf(1), Integer.valueOf(bArr.length));
        return fromByte(bArr[0]);
    }

    @Deprecated
    public static TraceOptions fromBytes(byte[] bArr, int i) {
        C2865Utils.checkIndex(i, bArr.length);
