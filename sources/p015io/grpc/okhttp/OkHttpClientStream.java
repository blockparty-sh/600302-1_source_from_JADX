...
import p015io.grpc.internal.AbstractClientStream;
import p015io.grpc.internal.ClientStreamListener.RpcProgress;
import p015io.grpc.internal.Http2ClientStreamTransportState;
...
class OkHttpClientStream extends AbstractClientStream {
    ...
    class TransportState extends Http2ClientStreamTransportState {
        ...
        public void start(int i) {
            Preconditions.checkState(OkHttpClientStream.this.f682id == -1, "the stream has been started with id %s", i);
            OkHttpClientStream.this.f682id = i;
            OkHttpClientStream.this.state.onStreamAllocated();
            if (this.canStart) {
                this.frameWriter.synStream(OkHttpClientStream.this.useGet, false, OkHttpClientStream.this.f682id, 0, this.requestHeaders);
                OkHttpClientStream.this.statsTraceCtx.clientOutboundHeaders();
                this.requestHeaders = null;
                if (this.pendingData.size() > 0) {
                    this.outboundFlow.data(this.pendingDataHasEndOfStream, OkHttpClientStream.this.f682id, this.pendingData, this.flushPendingData);
                }
                this.canStart = false;
            }
        }
        ...
        public void bytesRead(int i) {
            ...
            if (f <= ((float) i3) * 0.5f) {
                ...
                this.frameWriter.windowUpdate(OkHttpClientStream.this.mo36406id(), (long) i4);
            }
        }
        ...
        public void transportHeadersReceived(List<Header> list, boolean z) {
            if (z) {
                transportTrailersReceived(C2839Utils.convertTrailers(list));
            } else {
                transportHeadersReceived(C2839Utils.convertHeaders(list));
            }
        }
        ...
        public void transportDataReceived(Buffer buffer, boolean z) {
            ...
            if (this.window < 0) {
                this.frameWriter.rstStream(OkHttpClientStream.this.mo36406id(), ErrorCode.FLOW_CONTROL_ERROR);
                this.transport.finishStream(OkHttpClientStream.this.mo36406id(), Status.INTERNAL.withDescription("Received data size exceeded our receiving window size"), RpcProgress.PROCESSED, false, null, null);
                return;
            }
        ...
        private void onEndOfStream() {
            if (!isOutboundClosed()) {
                this.transport.finishStream(OkHttpClientStream.this.mo36406id(), null, RpcProgress.PROCESSED, false, ErrorCode.CANCEL, null);
            } else {
                this.transport.finishStream(OkHttpClientStream.this.mo36406id(), null, RpcProgress.PROCESSED, false, null, null);
            }
        }
        ...
        public void cancel(Status status, boolean z, Metadata metadata) {
            if (!this.cancelSent) {
                ...
                if (this.canStart) {
                    ...
                    this.transport.finishStream(OkHttpClientStream.this.mo36406id(), status, RpcProgress.PROCESSED, z, ErrorCode.CANCEL, metadata);
                }
            }
        }
        ...
        public void sendBuffer(Buffer buffer, boolean z, boolean z2) {
            if (!this.cancelSent) {
                if (this.canStart) {
                    ...
                    Preconditions.checkState(OkHttpClientStream.this.mo36406id() != -1, "streamId should be set");
                    this.outboundFlow.data(z, OkHttpClientStream.this.mo36406id(), buffer, z2);
                }
            }
        }
