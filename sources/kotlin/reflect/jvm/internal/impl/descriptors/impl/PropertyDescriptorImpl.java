...
public class PropertyDescriptorImpl extends VariableDescriptorWithInitializerImpl implements PropertyDescriptor {
    ...
    public PropertyDescriptor doSubstitute(@NotNull CopyConfiguration copyConfiguration) {
        ...
        if (receiverParameterDescriptor2 != null) {
            kotlinType = substituteTypeParameters.substitute(receiverParameterDescriptor2.getType(), Variance.IN_VARIANCE);
            if (kotlinType == null) {
                return null;
            }
        } else {
        ...
        createSubstitutedCopy.setType(substitute, (List<? extends TypeParameterDescriptor>) arrayList, receiverParameterDescriptor, kotlinType);
        ...
        PropertySetterDescriptor propertySetterDescriptor = this.setter;
        if (propertySetterDescriptor != null) {
            PropertySetterDescriptorImpl propertySetterDescriptorImpl2 = new PropertySetterDescriptorImpl(createSubstitutedCopy, propertySetterDescriptor.getAnnotations(), copyConfiguration.modality, normalizeVisibility(this.setter.getVisibility(), copyConfiguration.kind), this.setter.isDefault(), this.setter.isExternal(), this.setter.isInline(), copyConfiguration.kind, copyConfiguration.original == null ? null : copyConfiguration.original.getSetter(), SourceElement.NO_SOURCE);
        ...
        if (copyConfiguration.copyOverrides) {
            ...
            for (PropertyDescriptor substitute2 : getOverriddenDescriptors()) {
                create.add(substitute2.substitute(substituteTypeParameters));
            }
