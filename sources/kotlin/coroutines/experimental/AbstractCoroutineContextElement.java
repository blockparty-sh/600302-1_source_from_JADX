...
public abstract class AbstractCoroutineContextElement implements Element {
    @NotNull
    private final C2918Key<?> key;

    public AbstractCoroutineContextElement(@NotNull C2918Key<?> key2) {
    ...
    @Nullable
    public <E extends Element> E get(@NotNull C2918Key<E> key2) {
        Intrinsics.checkParameterIsNotNull(key2, "key");
        return DefaultImpls.get(this, key2);
    }

    @NotNull
    public C2918Key<?> getKey() {
        return this.key;
    }

    @NotNull
    public CoroutineContext minusKey(@NotNull C2918Key<?> key2) {
        Intrinsics.checkParameterIsNotNull(key2, "key");
