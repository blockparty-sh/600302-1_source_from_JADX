...
public abstract class AbstractReferenceMap<K, V> extends AbstractHashedMap<K, V> {
    ...
    protected static class ReferenceEntry<K, V> extends HashEntry<K, V> {
        ...
        public ReferenceEntry(AbstractReferenceMap<K, V> abstractReferenceMap, HashEntry<K, V> hashEntry, int i, K k, V v) {
    ...
    static class ReferenceEntrySet<K, V> extends EntrySet<K, V> {
        ...
        public <T> T[] toArray(T[] tArr) {
            ...
            while (it.hasNext()) {
                arrayList.add(new DefaultMapEntry((Entry) it.next()));
            }
            return arrayList.toArray(tArr);
        }
    }

    static class ReferenceEntrySetIterator<K, V> extends ReferenceBaseIterator<K, V> implements Iterator<Entry<K, V>> {
            ...
            if (hashEntry2 != null) {
                ...
                if (referenceEntry.purge(reference)) {
                    if (hashEntry4 == null) {
                        ...
                        hashEntry4.next = hashEntry2.next;
                    }
                    this.size--;
    ...
    /* access modifiers changed from: protected */
    public ReferenceEntry<K, V> createEntry(HashEntry<K, V> hashEntry, int i, K k, V v) {
        ReferenceEntry referenceEntry = new ReferenceEntry(this, hashEntry, i, k, v);
