...
public class ComputeInField {
    ...
    public short[] solveEquation(short[][] sArr, short[] sArr2) {
        try {
            if (sArr.length == sArr2.length) {
                this.f1543A = (short[][]) Array.newInstance(short.class, new int[]{sArr.length, sArr.length + 1});
                this.f1544x = new short[sArr.length];
                for (int i = 0; i < sArr.length; i++) {
                    for (int i2 = 0; i2 < sArr[0].length; i2++) {
                        this.f1543A[i][i2] = sArr[i][i2];
                    }
                }
                for (int i3 = 0; i3 < sArr2.length; i3++) {
                    this.f1543A[i3][sArr2.length] = GF2Field.addElem(sArr2[i3], this.f1543A[i3][sArr2.length]);
                }
    ...
    public short[][] inverse(short[][] sArr) {
        try {
            this.f1543A = (short[][]) Array.newInstance(short.class, new int[]{sArr.length, sArr.length * 2});
            if (sArr.length == sArr[0].length) {
                for (int i = 0; i < sArr.length; i++) {
                    ...
                    for (int length = sArr.length; length < sArr.length * 2; length++) {
                        this.f1543A[i][length] = 0;
                    }
                    this.f1543A[i][this.f1543A.length + i] = 1;
                }
                computeZerosUnder(true);
                for (int i3 = 0; i3 < this.f1543A.length; i3++) {
                    short invElem = GF2Field.invElem(this.f1543A[i3][i3]);
                    for (int i4 = i3; i4 < this.f1543A.length * 2; i4++) {
                        this.f1543A[i3][i4] = GF2Field.multElem(this.f1543A[i3][i4], invElem);
                    }
                }
                computeZerosAbove();
                short[][] sArr2 = (short[][]) Array.newInstance(short.class, new int[]{this.f1543A.length, this.f1543A.length});
                for (int i5 = 0; i5 < this.f1543A.length; i5++) {
                    for (int length2 = this.f1543A.length; length2 < this.f1543A.length * 2; length2++) {
                        sArr2[i5][length2 - this.f1543A.length] = this.f1543A[i5][length2];
                    }
                }
                return sArr2;
            }
    ...
    private void substitute() throws RuntimeException {
        ...
        if (invElem != 0) {
            short[] sArr2 = this.f1544x;
            short[][] sArr3 = this.f1543A;
            sArr2[sArr3.length - 1] = GF2Field.multElem(sArr3[sArr3.length - 1][sArr3.length], invElem);
            int length = this.f1543A.length - 2;
            while (length >= 0) {
                short[][] sArr4 = this.f1543A;
                short s = sArr4[length][sArr4.length];
                for (int length2 = sArr4.length - 1; length2 > length; length2--) {
                    s = GF2Field.addElem(s, GF2Field.multElem(this.f1543A[length][length2], this.f1544x[length2]));
                }
                short invElem2 = GF2Field.invElem(this.f1543A[length][length]);
                if (invElem2 != 0) {
                    this.f1544x[length] = GF2Field.multElem(s, invElem2);
                    length--;
                } else {
    ...
    public short[][] multiplyMatrix(short[][] sArr, short[][] sArr2) throws RuntimeException {
        if (sArr[0].length == sArr2.length) {
            this.f1543A = (short[][]) Array.newInstance(short.class, new int[]{sArr.length, sArr2[0].length});
            for (int i = 0; i < sArr.length; i++) {
