...
public class NTRUSigner {
    ...
    private byte[] signHash(byte[] bArr, NTRUSigningPrivateKeyParameters nTRUSigningPrivateKeyParameters) {
        ...
        } while (!verify(createMsgRep, sign, publicKey.f1528h));
        byte[] binary = sign.toBinary(this.params.f1525q);
    ...
    private IntegerPolynomial sign(IntegerPolynomial integerPolynomial, NTRUSigningPrivateKeyParameters nTRUSigningPrivateKeyParameters) {
        int i = this.params.f1520N;
        int i2 = this.params.f1525q;
        NTRUSigningPublicKeyParameters publicKey = nTRUSigningPrivateKeyParameters.getPublicKey();
        IntegerPolynomial integerPolynomial2 = new IntegerPolynomial(i);
        for (int i3 = this.params.f1519B; i3 >= 1; i3--) {
            Polynomial polynomial = nTRUSigningPrivateKeyParameters.getBasis(i3).f1526f;
            ...
            IntegerPolynomial integerPolynomial3 = (IntegerPolynomial) nTRUSigningPrivateKeyParameters.getBasis(i3).f1527h.clone();
            if (i3 > 1) {
                integerPolynomial3.sub(nTRUSigningPrivateKeyParameters.getBasis(i3 - 1).f1527h);
            } else {
                integerPolynomial3.sub(publicKey.f1528h);
            }
            integerPolynomial = mult2.mult(integerPolynomial3, i2);
        }
        Polynomial polynomial3 = nTRUSigningPrivateKeyParameters.getBasis(0).f1526f;
    ...
    private boolean verifyHash(byte[] bArr, byte[] bArr2, NTRUSigningPublicKeyParameters nTRUSigningPublicKeyParameters) {
        ...
        wrap.get(bArr3);
        return verify(createMsgRep(bArr, wrap.getInt()), IntegerPolynomial.fromBinary(bArr3, this.params.f1520N, this.params.f1525q), nTRUSigningPublicKeyParameters.f1528h);
    }
    ...
    public IntegerPolynomial createMsgRep(byte[] bArr, int i) {
        int i2 = this.params.f1520N;
        int numberOfLeadingZeros = 31 - Integer.numberOfLeadingZeros(this.params.f1525q);
