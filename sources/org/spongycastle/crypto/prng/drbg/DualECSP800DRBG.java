...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _P */
    private ECPoint f1331_P;

    /* renamed from: _Q */
    private ECPoint f1332_Q;
    private C3639Fp _curve;
        ...
        if (C3636Utils.isTooLarge(bArr, 512)) {
            ...
            while (true) {
                if (i2 == dualECPointsArr.length) {
                    ...
                    i2++;
                } else if (C3636Utils.getMaxSecurityStrength(digest) >= dualECPointsArr[i2].getSecurityStrength()) {
                    this._seedlen = dualECPointsArr[i2].getSeedLen();
                    this._outlen = dualECPointsArr[i2].getMaxOutlen() / 8;
                    this.f1331_P = dualECPointsArr[i2].getP();
                    this.f1332_Q = dualECPointsArr[i2].getQ();
                } else {
                    throw new IllegalArgumentException("Requested security strength is not supported by digest");
                }
            }
            if (this.f1331_P != null) {
                this.f1333_s = C3636Utils.hash_df(this._digest, concatenate, this._seedlen);
                this._sLength = this.f1333_s.length;
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (C3636Utils.isTooLarge(bArr2, 512)) {
            ...
            if (bArr2 != null) {
                bigInteger = new BigInteger(1, xor(this.f1333_s, C3636Utils.hash_df(this._digest, bArr2, this._seedlen)));
            } else {
                bigInteger = new BigInteger(1, this.f1333_s);
            }
            Arrays.fill(bArr, 0);
            BigInteger bigInteger2 = bigInteger;
            int i = 0;
            for (int i2 = 0; i2 < length2; i2++) {
                bigInteger2 = getScalarMultipleXCoord(this.f1331_P, bigInteger2);
                byte[] byteArray = getScalarMultipleXCoord(this.f1332_Q, bigInteger2).toByteArray();
            ...
            if (i < bArr.length) {
                bigInteger2 = getScalarMultipleXCoord(this.f1331_P, bigInteger2);
                byte[] byteArray2 = getScalarMultipleXCoord(this.f1332_Q, bigInteger2).toByteArray();
            ...
            this.f1333_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f1331_P, bigInteger2));
            return length;
        }
    }
    ...
    public void reseed(byte[] bArr) {
        if (!C3636Utils.isTooLarge(bArr, 512)) {
            this.f1333_s = C3636Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f1333_s, this._seedlen), this._entropySource.getEntropy(), bArr), this._seedlen);
            this._reseedCounter = 0;
            return;
        }
