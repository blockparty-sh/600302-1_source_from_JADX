...
public class RC2WrapEngine implements Wrapper {
    ...
    /* renamed from: iv */
    private byte[] f1149iv;
    private CipherParameters param;
    private ParametersWithIV paramPlusIV;
    Digest sha1 = new SHA1Digest();

    /* renamed from: sr */
    private SecureRandom f1150sr;
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (cipherParameters instanceof ParametersWithRandom) {
            ...
            this.f1150sr = parametersWithRandom.getRandom();
            cipherParameters = parametersWithRandom.getParameters();
        } else {
            this.f1150sr = new SecureRandom();
        }
        if (cipherParameters instanceof ParametersWithIV) {
            this.paramPlusIV = (ParametersWithIV) cipherParameters;
            this.f1149iv = this.paramPlusIV.getIV();
            this.param = this.paramPlusIV.getParameters();
            if (this.forWrapping) {
                byte[] bArr = this.f1149iv;
        ...
        if (this.forWrapping) {
            this.f1149iv = new byte[8];
            this.f1150sr.nextBytes(this.f1149iv);
            this.paramPlusIV = new ParametersWithIV(this.param, this.f1149iv);
        }
    }
    ...
    public byte[] wrap(byte[] bArr, int i, int i2) {
        if (this.forWrapping) {
            ...
            if (bArr4.length % this.engine.getBlockSize() == 0) {
                ...
                byte[] bArr6 = this.f1149iv;
                byte[] bArr7 = new byte[(bArr6.length + bArr5.length)];
                System.arraycopy(bArr6, 0, bArr7, 0, bArr6.length);
                System.arraycopy(bArr5, 0, bArr7, this.f1149iv.length, bArr5.length);
    ...
    public byte[] unwrap(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forWrapping) {
            ...
            System.arraycopy(bArr3, 0, this.f1149iv, 0, 8);
            ...
            if (!checkCMSKeyChecksum(bArr6, bArr7)) {
                ...
                sb.append("too many pad bytes (");
                sb.append(bArr6.length - ((bArr6[0] & 255) + 1));
                sb.append(")");
                throw new InvalidCipherTextException(sb.toString());
            }
        } else {
            StringBuilder sb2 = new StringBuilder();
            sb2.append("Ciphertext not multiple of ");
            sb2.append(this.engine.getBlockSize());
