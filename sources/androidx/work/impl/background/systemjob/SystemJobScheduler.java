...
public class SystemJobScheduler implements Scheduler {
    ...
    public void schedule(@NonNull WorkSpec... workSpecArr) {
        ...
        while (i2 < length) {
            ...
            try {
                WorkSpec workSpec2 = workDatabase.workSpecDao().getWorkSpec(workSpec.f87id);
                ...
                if (workSpec2 == null) {
                    ...
                    sb2.append(str);
                    sb2.append(workSpec.f87id);
                    sb2.append(" because it is no longer enqueued");
                    logger2.warning(str3, sb2.toString(), new Throwable[0]);
                    workDatabase.setTransactionSuccessful();
                } else {
                    SystemIdInfo systemIdInfo = workDatabase.systemIdInfoDao().getSystemIdInfo(workSpec.f87id);
                    ...
                    if (systemIdInfo == null) {
                        this.mWorkManager.getWorkDatabase().systemIdInfoDao().insertSystemIdInfo(new SystemIdInfo(workSpec.f87id, nextJobSchedulerIdWithRange));
                    }
                    scheduleInternal(workSpec, nextJobSchedulerIdWithRange);
                    if (VERSION.SDK_INT == 23) {
                        List pendingJobIds = getPendingJobIds(this.mContext, this.mJobScheduler, workSpec.f87id);
    ...
    public void scheduleInternal(WorkSpec workSpec, int i) {
        ...
        Logger.get().debug(TAG, String.format("Scheduling work ID %s Job ID %s", new Object[]{workSpec.f87id, Integer.valueOf(i)}), new Throwable[0]);
        try {
