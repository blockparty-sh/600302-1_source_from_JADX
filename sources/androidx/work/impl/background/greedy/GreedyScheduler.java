        ...
        for (WorkSpec workSpec : workSpecArr) {
            if (workSpec.state == State.ENQUEUED && !workSpec.isPeriodic() && workSpec.initialDelay == 0 && !workSpec.isBackedOff()) {
                if (!workSpec.hasConstraints()) {
                    Logger.get().debug(TAG, String.format("Starting work for %s", new Object[]{workSpec.f87id}), new Throwable[0]);
                    this.mWorkManagerImpl.startWork(workSpec.f87id);
                } else if (VERSION.SDK_INT < 24 || !workSpec.constraints.hasContentUriTriggers()) {
                    arrayList.add(workSpec);
                    arrayList2.add(workSpec.f87id);
                }
            }
        }
    ...
    private void removeConstraintTrackingFor(@NonNull String str) {
        synchronized (this.mLock) {
            ...
            while (true) {
                if (i >= size) {
                    break;
                } else if (((WorkSpec) this.mConstrainedWorkSpecs.get(i)).f87id.equals(str)) {
